...
const OWNER_NUMBER = "2348155033420";
const OWNER_NAME = "Knox The Great";
...
${argText || "Everyone, KNOX BOT has a message for you!"}`
    );
    return;
  }
...
5. Keep the chat useful.`
  ...
  if (command === "welcome") {
    await sendText(
      number,
      "👋 Welcome to the group! Powered by 👑 KNOX BOT."
    );
    return;
  }
...
Commands: 100`
  ...
  if (command === "goodmorning") {
    await sendText(
      number,
      "🌅 Good morning! Have a great day from 👑 KNOX BOT."
    );
    return;
  }
  ...
  if (command === "goodnight") {
    await sendText(
      number,
      "🌙 Good night! Rest well from 👑 KNOX BOT."
    );
    return;
  }
  ...
  if (command === "motivate") {
    await sendText(
      number,
      "🔥 Keep building. Keep learning. Don't stop improving."
    );
    return;
  }
